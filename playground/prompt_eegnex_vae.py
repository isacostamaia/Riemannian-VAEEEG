...
from playground.prompt import PromptPool, PromptPool_WP, prepend_prompts_to_vector
...
class VariationalEncoder(nn.Module):
    def __init__(self, latent_dims, x_shape):
        ...
        self.eegnex = EEGNeX(n_chans=x_shape[-2], # 129 channels
                n_outputs= self.embed_dim, # 128 output for learning z mean and logsigma
        ...
        prompt_length = 2
        top_k = 3
        prompt_x_concat_dim = self.embed_dim + prompt_length*prompt_dim*top_k

        #output prompt will be (batch_size, prompt_length*top_k, prompt_dim)
        self.pool = PromptPool(num_prompts=10, prompt_dim=prompt_dim, prompt_length=prompt_length, key_dim=self.embed_dim, top_k=top_k)
        # #output prompt will be (batch_size, prompt_length, prompt_dim)
        # self.pool = PromptPool_WP(num_prompts=10, prompt_dim=8, prompt_length=4, key_dim=self.embed_dim, top_k=3)

        self.linear2 = nn.Linear(prompt_x_concat_dim, latent_dims)
        self.linear3 = nn.Linear(prompt_x_concat_dim, latent_dims)
    ...
    def forward(self, x):
        ...
        x = self.eegnex(x)
        prompts = self.pool(x)
        x = prepend_prompts_to_vector(x, prompts)
        mu =  self.linear2(x)
...
class PromptEEGNeXVariationalAutoencoder(nn.Module):
    def __init__(self, latent_dims, x_shape, pred_dim):
        super(PromptEEGNeXVariationalAutoencoder, self).__init__()
