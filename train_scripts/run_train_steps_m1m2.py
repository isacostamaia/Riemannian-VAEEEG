...
####################################TRAIN M1#############################################
    ...
    exp_params = {"seed": seed,
                ...
                #   linear_annealing(n_iter=m1_params["num_epochs"], n_warmup=10)
    ...
    for epoch in range(m1_params["num_epochs"]):
        train_m1(m1, train_loader, exp_params["beta"][epoch], epoch, loss_dict, optimizer)
    end = time.time()
    ...
    def create_exp_folder(db_name, timestamp):
        os.makedirs(db_name, exist_ok=True)
        folder = os.path.join(db_name, exp_subfolder, f"exp_{timestamp}", exp_step)
        os.makedirs(folder)
        print(f"Created folder: {folder}")
        return folder

    exp_path = create_exp_folder(os.path.join("experiences", exp_params["db_name"]), timestamp)
    ...
    ####Plots and Inference####
    from visualizations.inference import m1_infer, plot_2d, plot_3d
    from umap import UMAP
    
    m1.eval()

    ##M1 train: variables and plots
    x_train, z_train, z_mean_train, labels_train, domains_train, _ = m1_infer(m1, train_loader)
    ...
    ##M1test: variables and plots
    x_test, z_test, z_mean_test, labels_test, domains_test, _ = m1_infer(m1, test_loader)
    ...
    with open(os.path.join(exp_path, 'report.txt'), "a") as f:
        ...
        print("\n\n", file=f)
    #project and plot 2D and 3D test embedding space with Linear probe predictions
...
#######################################FINETUNE M2######################
    ...
    # m1.load_state_dict(torch.load("experiences/EEG/M1+M2_exp_20251002_143110/m1.pth"))
    # m2.load_state_dict(torch.load("experiences/EEG/M1+M2_exp_20251002_143110/m2.pth"))

    # exp_params["parent_exp_path"] = "experiences/EEG/M1+M2_exp_20251002_143110"
    ...
    #Training loop
    start = time.time()
    for epoch in range(exp_params["m2_finetune_epochs"]):
        freeze_m1_train_m2(m1, m2, train_loader, loss_dict, optimizer)
